class TerminalApp {
    ...
    connectSocket() {
        ...
        this.socket.on('session-joined', ({ sessionId, log }) => {
            this.createTerminalSession(sessionId, log);
        });
        ...
        this.socket.on('sessions-list', (sessions) => {
            ...
            sessions.forEach(session => {
                if(session.active && !this.sessions.has(session.sessionId)) {
                    this.socket.emit('join-session', { sessionId: session.sessionId });
                }
            });
        });
    }
    ...
    createTerminalSession(sessionId, logData = '') {
        ...
        sessions.forEach(session => {
            ...
            sessionDiv.innerHTML = `
                <div class="session-info">
                    ...
                    <small>${session.active ? 'Active' : 'Saved'} | Created: ${new Date(session.createdAt).toLocaleString()}</small>
                </div>
                <div class="session-actions">
                    <button class="btn-small" onclick="app.joinSession('${session.sessionId}'); app.closeModal('sessionsModal')">
                        ${session.active ? 'Open' : 'Join'}
                    </button>
